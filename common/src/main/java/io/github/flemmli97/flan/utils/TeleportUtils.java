        ...
        if (northDist > southDist) {
            if (eastDist > westDist) {
                if (southDist > westDist)
                    return new Pair<>(Direction.WEST, new Vec3(dim.minX - 1.5, from.y(), from.z()));
                return new Pair<>(Direction.SOUTH, new Vec3(from.x(), from.y(), dim.maxZ + 1.5));
            }
            if (southDist > eastDist)
                return new Pair<>(Direction.EAST, new Vec3(dim.maxX + 1.5, from.y(), from.z()));
            return new Pair<>(Direction.SOUTH, new Vec3(from.x(), from.y(), dim.maxZ + 1.5));
        }
        if (eastDist > westDist) {
            if (northDist > westDist)
                return new Pair<>(Direction.WEST, new Vec3(dim.minX - 1.5, from.y(), from.z()));
            return new Pair<>(Direction.NORTH, new Vec3(from.x(), from.y(), dim.minZ - 1.5));
        }
